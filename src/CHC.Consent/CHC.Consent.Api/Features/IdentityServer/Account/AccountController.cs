...
namespace CHC.Consent.Api.Features.IdentityServer
{
    ...
    {
        private readonly UserManager<ConsentUser> _userManager;
        private readonly SignInManager<ConsentUser> _signInManager;
        ...
        public AccountController(
            ...
            SignInManager<ConsentUser> signInManager,
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _events.RaiseAsync(new UserLoginSuccessEvent(user.UserName, user.Id.ToString(CultureInfo.InvariantCulture), user.UserName));
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            ...
            var name = principal.FindFirst(JwtClaimTypes.Name)?.Value ?? user.SubjectId;
            await _events.RaiseAsync(new UserLoginSuccessEvent(provider, providerUserId, user.SubjectId, name));
            await HttpContext.SignInAsync(user.SubjectId, name, provider, localSignInProps, additionalLocalClaims.ToArray());
        ...
        private async Task<LoginViewModel> BuildLoginViewModelAsync(string returnUrl)
        {
            ...
            var providers = schemes
                ...
                .Where(x => x.Name != OpenIdConnectDefaults.DisplayName)
                .Select(
        ...
        private async Task<(ConsentUser user, string provider, string providerUserId, IEnumerable<Claim> claims)>
        ...
        private void ProcessLoginCallbackForOidc(
            AuthenticateResult externalResult, List<Claim> localClaims, AuthenticationProperties localSignInProps)
        {
